...
@ResultPath(value="/")
public class RejectRequestForProjectAction  extends ActionSupport  {
	private static final long serialVersionUID = 1L;
	private String rid = "-1";
	private String pid = "-1";
	
	@SuppressWarnings("rawtypes")
	@Action(value="rejectRequestForProject", results={
	...
	public String execute(){
		...
		try {
			...
			if(!request.getIsCancelledBySource()){
				...
				addActionMessage("درخواست عضویت انتخاب شده رد شد.");
			}else{
				project.getRecivedRequests().remove(request.getId());
				session.update(project);
